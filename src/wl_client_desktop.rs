...
use wayland_client::{
    ...
    protocol::{wl_registry::WlRegistry, wl_output::{WlOutput, Transform}}, Proxy, WEnum,
};
...
pub struct OutputState {
    ...
    transform: WEnum<Transform>, // TODO: support upright displays
    done: bool,
}

pub struct WlClientDesktopState {
...
impl WlClientDesktopState {
    pub fn new() -> Self {
        let connection = Connection::connect_to_env().expect("wayland connection");
        let (globals, mut queue) = registry_queue_init::<Self>(&connection).expect("wayland globals");
        let qh = queue.handle();

        
        let mut state = Self { 
            connection, 
            xdg_output_mgr: globals.bind(&qh, 2..=3, ()).expect(ZxdgOutputManagerV1::interface().name),
...
impl Dispatch<ZxdgOutputV1, u32> for WlClientDesktopState {
    fn event(
        state: &mut Self,
        _proxy: &ZxdgOutputV1,
...
impl Dispatch<WlOutput, u32> for WlClientDesktopState {
   fn event(
           state: &mut Self,
           _proxy: &WlOutput,
        ...
        match event {
            ...
            wayland_client::protocol::wl_output::Event::Geometry { model, transform, .. } => {
                if let Some(output) = state.outputs.iter_mut().find(|o| o.id == *data) {
                    output.model = model;
...
impl Dispatch<WlRegistry, ()> for WlClientDesktopState {
    fn event(
        _state: &mut Self,
        _proxy: &WlRegistry,
...
impl Dispatch<ZxdgOutputManagerV1, ()> for WlClientDesktopState {
    fn event(
        _state: &mut Self,
...
impl Dispatch<ZwlrExportDmabufManagerV1, ()> for WlClientDesktopState {
    fn event(
        _state: &mut Self,
...
impl Dispatch<WlRegistry, GlobalListContents> for WlClientDesktopState {
    fn event(
        _state: &mut Self,
        _proxy: &WlRegistry,
        _event: <WlRegistry as Proxy>::Event,
